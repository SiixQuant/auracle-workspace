...
### Changed
- The Auracle Plan is now grouped the way it says it is. Each of the four headings — Research, Build · Test, Operate and System — sits over a column of its own rooms rather than over one flat row of eleven cards, so a room is under the group it belongs to, every card is wide enough to write its name out in full, and the whole plan is a block that fills a tall pane instead of a strip across the top of it. The wires follow the new shape: work hands off across the channel between two groups, runs straight down a group's own column between neighbouring rooms, and steps around a room it does not touch rather than being drawn through it, with the alert's pointer coming down beside the room it names. On the Board, a pane that is tall for the pipeline in it now lets that pipeline wrap onto a second band — sources and questions above, everything they produced below — while a wide pane keeps the single left-to-right row exactly as before. Nothing has moved between groups, and every reading, click and keyboard route is unchanged.
- The Auracle Board and the Plan now open at a size that fits the pane they are in. A board or a floor plan with room to spare is drawn larger instead of sitting as a small block in the middle of an empty canvas, up to a comfortable limit, and one too big for the pane is still brought down whole as before. Everything down the middle of the Board lines up on one centre now — the line that says what you are working on, the buttons under it, and the columns of cards below them — with the space between them set on a single rhythm. Zooming, panning and the fit button behave exactly as they did.
...
### Fixed
- Buttons, toggles, checkboxes, and avatars are readable again on the black-and-white palette: anything drawn on the white primary now takes its ink from the palette instead of assuming white.
...
- Live Algorithms now chart each deployment's equity over time, reconstructed from its fills and marked to daily closes.
