...
3. Write ONE new strategy file under `strategies/` implementing the platform's Strategy contract (`universe`, `prices_to_signals`, optionally `signals_to_target_weights` — conventions in the auracle-quant skill). The `universe` MUST be drawn only from symbols this install can backtest: call the `data_coverage` tool first and pick from its `backtestable` set — never invent tickers or use instruments with no ingested data (`run_backtest_now` refuses them, and bare futures/commodity roots aren't available via the yfinance ingest path). If the paper's instruments aren't available, adapt the idea to the closest available symbols or stop and say exactly what would need ingesting. The module docstring must state the tradable hypothesis as a measurable statistical phenomenon (what is computed, over what horizon, with what expected sign) and cite the paper: source, paper id, title, and URL. Sensible parameter defaults, no lookahead, no fabricated numbers.
