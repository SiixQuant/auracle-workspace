...
export function ValidationPanel(_props: PanelHostProps): JSX.Element {
  const [strategies] = usePolledFeed<{
    strategies?: Array<{ path?: string; strategy_path?: string; name?: string }>;
  }>('/ui/api/backtest/strategies', 60_000);
  const [selected, setSelected] = useState('');
  const [verdict, setVerdict] = useState<
    { summary?: string; signals?: ValidationSignal[]; detail?: string } | null | undefined
  >();

  const run = async (path: string) => {
    setVerdict(undefined);
    setVerdict(
      await getJson(`/ui/api/validation?strategy_path=${encodeURIComponent(path)}`)
    );
  };

  if (strategies === null) return <div style={styles.page}><Placeholder state="unreachable" what="strategies" /></div>;
  const options = (strategies?.strategies ?? [])
    .map((s) => s.strategy_path ?? s.path ?? '')
    .filter((p) => p.length > 0);
  return (
    <div style={styles.page}>
      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
        ...
        >
          <option value="">Pick a strategy to check for overfitting…</option>
          {options.map((path) => (
      ...
      {selected && verdict === undefined ? <Placeholder state="loading" what="validation" /> : null}
      {selected && verdict === null ? <Placeholder state="unreachable" what="validation" /> : null}
      {verdict?.detail ? <div style={styles.error}>{verdict.detail}</div> : null}
      {verdict?.summary ? <div>{verdict.summary}</div> : null}
      {(verdict?.signals ?? []).map((signal, index) => (
        <div key={signal.name ?? index} style={styles.row}>
          <span style={styles.chip(signal.tier === 'pass' ? 'ok' : signal.tier === 'warn' ? 'warn' : 'muted')}>
            {signal.tier ?? '—'}
          </span>
          <span>
            {signal.plain ?? signal.name}
            {signal.fix ? <span style={styles.note}> Fix: {signal.fix}</span> : null}
          </span>
        </div>
      ))}
    </div>
